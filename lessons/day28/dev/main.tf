...
# Store AKS admin kubeconfig in Key Vault (for emergency access)
resource "azurerm_key_vault_secret" "aks_admin_kubeconfig" {
  count        = var.enable_key_vault ? 1 : 0
  name         = "aks-admin-kubeconfig"
  ...
  lifecycle {
    # Allow deletion of secrets when infrastructure is destroyed
    prevent_destroy = false
    # Recreate if key vault or cluster changes
    replace_triggered_by = [azurerm_key_vault.main[0].id, azurerm_kubernetes_cluster.main.id]
  }

  depends_on = [azurerm_key_vault.main, azurerm_kubernetes_cluster.main]
}

# Store AKS cluster CA certificate
resource "azurerm_key_vault_secret" "aks_ca_certificate" {
  count        = var.enable_key_vault ? 1 : 0
  name         = "aks-ca-certificate"
  value        = base64encode(azurerm_kubernetes_cluster.main.kube_config.0.cluster_ca_certificate)
  key_vault_id = azurerm_key_vault.main[0].id
...
# Store cluster identity information
resource "azurerm_key_vault_secret" "cluster_identity_principal_id" {
  count        = var.enable_key_vault ? 1 : 0
  name         = "cluster-identity-principal-id"
...
# Store cluster identity tenant ID
resource "azurerm_key_vault_secret" "cluster_identity_tenant_id" {
  count        = var.enable_key_vault ? 1 : 0
  name         = "cluster-identity-tenant-id"
